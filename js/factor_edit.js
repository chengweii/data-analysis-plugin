...
function syncRelations(name_en, expression, callback) {
	...
	util.dao.execute(query_sql, [ name_en ], function(tx, res) {
		if (res.rows.length) {
			...
			util.dao.execute("select id from analysis_factor where name_en in ('"+factors.join("','")+"') and id not in (select relation_object_id from analysis_relations where object_id = ? and object_type = '2' and relation_object_type = '2')", [ object_id ], function(tx, res) {
				var rows = [];
				if (res.rows.length) {
